...
pair<long long,long long> segtree[200003];
...
pair<long long,long long> rmq(int p,int L,int R,int S,int T){
	...
	printf("(%d,%d)(%d,%d)[%d,%d]=%lld,%lld\n",L,M,M+1,R,p,idx,ret.first,ret.second);
	assert(ret.first>=0);
	return ret;
}
...
void build(int p,int L,int R){
	...
	printf("(%d,%d)[%d]=%lld,%lld\n",L,R,p,segtree[p].first,segtree[p].second);
